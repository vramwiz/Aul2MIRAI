...
| `preview_create_object_from_alias` | 完全なエイリアス1個の生成内容と空き位置を検証する。 |
| `create_object_from_alias` | 検証済みエイリアスから1個のオブジェクトを生成する。 |
...
`preview_create_object_from_alias`と`create_object_from_alias`は、最新の`state_token`、0-basedの`layer`と`frame`、正の`length`、1 MiB以下のUTF-8 `alias`を要求する。実生成では`apply: true`も必須とする。対象は1要求につき1個で、生成先が既存オブジェクトと重なる場合は`destination_occupied`として生成前に拒否する。

`alias`は`[0]`本体、`layer`、`frame`、連番の効果ブロックを含む1個分の完全な形式を渡す。AviUtl2の`create_object_from_alias`は効果ブロックだけの断片を受理しない。要求の`layer`、`frame`、`length`を配置の正本とし、エイリアス側の本体ヘッダーも同じ配置を記述する。

編集コールバック内でSDKの`create_object_from_alias`を呼び、生成直後に配置範囲、主効果、素材パス、効果順、全設定項目名と値を読み直す。相違または読み取り失敗があれば同じコールバック内で生成物を削除する。成功応答にはエイリアスSHA-256、UTF-8バイト数、効果順、素材パス、応答時点の`created_index`、検証項目を返す。
